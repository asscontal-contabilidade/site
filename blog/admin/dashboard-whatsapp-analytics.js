(() => {
  ...
  panel.innerHTML=`
    ...
    <div id="waAnalyticsDiagnostic" class="wa-analytics-diagnostic" hidden></div>
  ...
  async function load(period){
    ...
    const diagnostic=document.getElementById('waAnalyticsDiagnostic');
    try{
      ...
      const articleClicks=Number(data.totalClicks||0),siteClicks=Number(data.siteClicks ?? articleClicks),views=Number(data.totalViews||0);
      document.getElementById('waClicks').textContent=siteClicks.toLocaleString('pt-BR');
      ...
      const orphanClicks=Math.max(0,siteClicks-articleClicks);
      if(orphanClicks>0){
        diagnostic.hidden=false;
        diagnostic.innerHTML=`Foram registrados <b>${orphanClicks.toLocaleString('pt-BR')}</b> clique${orphanClicks===1?'':'s'} no WhatsApp sem vínculo com uma matéria. Isso indica que o envio ao Worker está funcionando, mas o identificador da matéria não está chegando corretamente.`;
      }else{
