(() => {
  ...
  panel.innerHTML=`
    ...
    <div class="wa-analytics-grid">
      <div class="wa-analytics-card"><span>Cliques totais</span><b id="waClicks">—</b><small>Todo o blog</small></div>
      <div class="wa-analytics-card"><span>Em matérias</span><b id="waArticleClicks">—</b><small>Cliques vinculados a artigos</small></div>
      <div class="wa-analytics-card"><span>Outras páginas</span><b id="waOtherClicks">—</b><small>Home, busca e categorias</small></div>
      <div class="wa-analytics-card"><span>Conversão</span><b id="waConversion">—</b><small>Cliques em matérias ÷ visualizações</small></div>
      <div class="wa-analytics-card"><span>Barra</span><b id="waBarClicks">—</b><small>CTA inferior nas matérias</small></div>
  ...
  async function load(period){
    ...
    const note=document.getElementById('waAnalyticsNote');
    try{
      ...
      const articleClicks=Number(data.totalClicks||0),siteClicks=Number(data.siteClicks ?? articleClicks),views=Number(data.totalViews||0),otherClicks=Math.max(0,siteClicks-articleClicks);
      document.getElementById('waClicks').textContent=siteClicks.toLocaleString('pt-BR');
      document.getElementById('waArticleClicks').textContent=articleClicks.toLocaleString('pt-BR');
      document.getElementById('waOtherClicks').textContent=otherClicks.toLocaleString('pt-BR');
